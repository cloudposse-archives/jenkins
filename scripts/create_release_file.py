...
def update_m2a_ci(build_version):
    ...
    for url in git_urls:
        ...
        modules_data[url] = sha.split('\t', 1)[0]
    create_release_file(modules_data, 'm2aci', build_version)
    git_runner.push_changes_to_m2a_releases()
...
def get_url_from_source(source):
    ...
    :param source: a source from the module.tf file
    :return: the modules git url
    """
    try:
...
def create_release_file(module_data, environment, version):
    ...
    """
    with open("/m2a-releases/{}/{}.json".format(environment, version), "w+") as new_file:
...
class GitRunner:
    ...
    def _create_cmd_for_adding_file(self):
        return [
            self.path_to_binary,
            "--git-dir=m2a-releases.git",
            "--work-tree=m2a-releases/",
            "add", ".",
        ]

    def _create_cmd_for_committing_file(self):
        return [
            self.path_to_binary,
            "--git-dir=m2a-releases.git",
            "--work-tree=m2a-releases/",
    ...
    def _create_cmd_for_pushing(self):
        return [
            self.path_to_binary,
            "--git-dir=m2a-releases.git",
            "--work-tree=m2a-releases/",
    ...
    def push_changes_to_m2a_releases(self):
        self.run_cmd(self._create_cmd_for_adding_file)
        self.run_cmd(self._create_cmd_for_committing_file)
        self.run_cmd(self._create_cmd_for_pushing)


if target_env is None:
    update_m2a_ci(version)
else:
